...
<!-- NOMOR -->
<div class="nomor">Nomor: 095/PENERIMAAN/{{ date('Y') }}</div>
...
<!-- DATA SISWA -->
<table class="tabel">
<tr><td class="label">Nama Lengkap</td><td class="nilai">{{ $formulir->nama_lengkap }}</td></tr>
<tr><td class="label">NISN</td><td class="nilai">{{ $formulir->nisn ?? '-' }}</td></tr>
<tr><td class="label">NIK</td><td class="nilai">{{ $formulir->nik ?? '-' }}</td></tr>
<tr><td class="label">Tempat/Tgl Lahir</td><td class="nilai">{{ $formulir->tempat_lahir }}, {{ \Carbon\Carbon::parse($formulir->tanggal_lahir)->format('d F Y') }}</td></tr>
<tr><td class="label">Jenis Kelamin</td><td class="nilai">{{ $formulir->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td></tr>
<tr><td class="label">Asal Sekolah</td><td class="nilai">{{ $formulir->asal_sekolah }}</td></tr>
<tr><td class="label">Jurusan</td><td class="nilai">{{ $formulir->jurusan->nama_jurusan ?? '-' }}</td></tr>
<tr><td class="label">Agama</td><td class="nilai">{{ ucfirst($formulir->agama ?? '-') }}</td></tr>
</table>

<!-- PERNYATAAN DITERIMA -->
<p class="isi">Berdasarkan hasil seleksi dan evaluasi yang telah dilakukan, dengan ini kami beritahukan bahwa calon siswa di atas <strong>DITERIMA</strong> sebagai siswa baru di SMK Antartika 1 untuk Tahun Pelajaran {{ date('Y') }}/{{ date('Y') + 1 }}.</p>
...
<!-- TANGGAL -->
<p class="kanan">Pasuruan, {{ \Carbon\Carbon::now()->format('d F Y') }}</p>
