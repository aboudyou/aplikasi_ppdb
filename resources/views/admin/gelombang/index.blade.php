                    ...
                    <table class="table table-hover align-middle mb-0" id="gelombangTable">
                        ...
                        <tbody>
                            ...
                            <tr class="border-bottom border-light">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-sm me-3" style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; font-weight: bold;">
                                            {{ strtoupper(substr($item->nama_gelombang, 0, 1)) }}
                                        </div>
                                        <div>
                                            <h6 class="mb-0 fw-bold">{{ $item->nama_gelombang }}</h6>
                                            @if($item->keterangan)
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <small class="text-muted mb-1">
                                            <i class="ti ti-calendar me-1"></i>{{ \Carbon\Carbon::parse($item->tanggal_mulai)->format('d M Y') }}
                                        </small>
                                        <small class="text-muted">
                                            <i class="ti ti-calendar-off me-1"></i>{{ \Carbon\Carbon::parse($item->tanggal_selesai)->format('d M Y') }}
                                        </small>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="fw-bold text-success">Rp {{ number_format($item->nilai ?? 0, 0, ',', '.') }}</span>
                                        @if($item->jenis_promo)
                                ...
                                <td>
                                    @if($item->kuota_maksimal > 0)
                                        <div class="d-flex flex-column gap-1">
                                            <div class="progress" style="height: 6px; width: 80px;">
                                                <div class="progress-bar {{ $item->getJumlahPeserta() >= $item->kuota_maksimal ? 'bg-danger' : 'bg-primary' }}"
                                                     style="width: {{ $item->kuota_maksimal > 0 ? min(($item->getJumlahPeserta() / $item->kuota_maksimal) * 100, 100) : 0 }}%"></div>
                                            </div>
                                            <small class="text-muted fw-semibold">{{ $item->getJumlahPeserta() }}/{{ $item->kuota_maksimal }}</small>
                                        </div>
                                    @else
                                    ...
                                    @if($item->jenis_promo)
                                        <div class="d-flex flex-column gap-1">
                                            <span class="badge bg-warning bg-opacity-10 text-warning border-0">
                                                <i class="ti ti-discount me-1"></i>{{ ucfirst($item->jenis_promo) }}
                                            </span>
                                            @if($item->nilai_promo > 0)
                                                <small class="text-muted fw-semibold">
                                                    {{ $item->tipe_nilai_promo === 'persen' ? $item->nilai_promo . '%' : 'Rp ' . number_format($item->nilai_promo, 0, ',', '.') }}
                                                </small>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="{{ route('admin.gelombang.edit', $item->id) }}"
                                           class="btn btn-outline-primary btn-sm"
                                           data-bs-toggle="tooltip" title="Edit Gelombang">
                                            <i class="ti ti-edit me-1"></i>Edit
                                        </a>
                                        <form action="{{ route('admin.gelombang.destroy', $item->id) }}" method="POST" class="d-inline"
